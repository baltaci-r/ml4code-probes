...
from .parser_utils import ParseResult, ParsedToken, tree_to_variable_index
...
def getDFG(lang, parse_result: ParseResult) -> List[DFGEdge]:
    """
    Returns list of ((dstTok, dstTokIdx), (srcTok, srcTokIdx)))
    There's no edge-type for simplicity.
    
    It's available, but removed from the final output for now.
    """
    if lang == 'python':
        dfg_fn = DFG_python
    elif lang == 'java':
    ...
    # this fellow returns 'dataflow' for all identifiers, doesn't know to differentiate variable names
    # from other tokens. This is just an approximate version of DFG, but it's not too bad
    dfg_raw = dfg_fn(parse_result.tree.root_node, parse_result.loc2tok_legacy, {})[0]
    dfg_raw = sorted(dfg_raw, key=lambda x: x[1])

    # we're now removing dangling nodes that aren't used anywhere.
    # as a side-effect, it removes non-variable nodes.
    indexs=set()
    for d in dfg_raw:
        if len(d[-1])!=0:
            indexs.add(d[1])
    ...
    for d in dfg_raw:
        if d[1] in indexs:
            if len(d[-1]) == len(d[-2]):
                #for a, b in zip(d[-2], d[-1]):
                for b in d[-1]:
                    dst = parse_result.toks[d[1]]
                    src = parse_result.toks[b]
                    #result.append(((d[0], d[1]), (a, b)))
                    result.append(DFGEdge(src, dst))
            else:
                assert len(d[-2]) == 1
                for b in d[-1]:
                    # result.append(((d[0], d[1]), (d[-2][0], b)))
                    dst = parse_result.toks[d[1]]
                    src = parse_result.toks[b]
                    result.append(DFGEdge(src, dst))
            #result.append(d)

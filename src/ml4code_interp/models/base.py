...
class InterpretableModel(object):
    ...
    def get_embeddings_per_token_av(self, parse_result: ParseResult) -> List[List[torch.Tensor]]:
        """
        Returns layer-wise embeddings for each ParsedToken. Since a ParsedToken may have multiple embeddings,
        because a ParsedToken may be split into multiple ModelTokens, the final shape of the output is
    ...
    def get_embeddings_batch(self, batch):
        ...
        hidden_outputs = [torch.split(out.squeeze(0), 1) for out in model_output.hidden_states]
    ...
    def _prepare_input(self, parse_result: ParseResult) -> BatchEncoding:
        ...
        """
        # if max_length:
        #     return self.tokenizer(parse_result.code, return_tensors='pt', max_length=max_length, padding='max_length')
        # else:
